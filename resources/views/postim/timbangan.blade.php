...
<main class="main-content position-relative border-radius-lg ">
    ...
    <div class="container">
        <div class="row">
            <div class="col-md-8  ">
                <h1>Timbangan Anak</h1>
                <a href="{{ url ('/timbangan/create') }}" class="btn btn-primary mt-3"><i class="fa-solid fa-plus"></i> Tambah Agenda</a>
            </div>
            ...
            <table class=" table text-center">
                ...
                @foreach($data as $dataTimb)
                <tr>
                    <td>{{ $dataTimb->id }}</td>
                    <td>{{ $dataTimb->nama_anak }}</td>
                    <td>{{ $dataTimb->umur }}</td>
                    <td>{{ $dataTimb->tgl_timbang }}</td>
                    <td>{{ $dataTimb->status_gizi }}</td>
                    <td>
                        <a href="{{ url ('/timbangan/lihat/' .$dataTimb->id) }}" class="btn btn-primary"><i class="fa-solid fa-eye"></i></a>
                        <a href="{{ url ('/timbangan/show/' .$dataTimb->id) }}" class="btn btn-warning"><i class="fa-solid fa-pencil"></i></a>
                        <a href="{{ url('/timbangan/destroy/'.$dataTimb->id) }}" " 
